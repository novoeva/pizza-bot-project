...
**Definition:** A chatbot talks about doing something; an agent actually does it: it plans what needs to happen, acts, checks the result, and keeps going (or re-plans) until the job's done — no human in between.
...
**Academy sim:** *Agentiness Spectrum* (sort systems from "pure tool" to "autonomous agent" — classification) + *Loop Stepper* (click through perceive→plan→act→observe — guided walkthrough). Verdict: sorting teaches recognizing the definition, not feeling the difference; the stepper is passive. Neither fits the 90-second persona.

**Correction, July 2026 (v1 "All talk" rejected, rebuild pending):** v1 had round 1 as a scripted chatbot dialogue (rejected separately for being confusing UX — tap-through bubbles with no clear "watch for this") and round 2 as four fixed action steps the *player* tapped in order. By Anthropic's own architectural distinction ("Building Effective Agents," Dec 2024): a **workflow** is steps orchestrated through a predefined path; an **agent** is a system that dynamically directs its own steps and tool use. Round 2 as built was a workflow the player operated by hand — the opposite of what the game claimed to teach. Every serious explainer of agents (Anthropic, OpenAI's agent-building guide, general "agent loop" writing) also agrees on a second point: an agent needs a **well-defined goal** before it can plan at all — vague goals make it stall or improvise badly. Evi independently converged on both points and supplied the reference model: a plan → act (tools) → reflect loop that branches back to "plan" when a result isn't OK, only reaching "response" once it is. This is the standard three-node agent loop (planning / tool use / reflection) used across the industry, e.g. as shown in the "Agentic Workflows" diagram Evi referenced.

**Correction, July 2026 (v2 "The loop" rejected in review, rebuilt as "From chat to agent"):** The plan→act→reflect→re-plan loop above was built, but playtested badly: the player taps "Run" once and then only *watches* nodes blink, with nothing to think about — "a passive, confusing game." The self-written-plan framing and the auto-running loop didn't land. Going back to basics, Evi reframed the point for a layperson to the simplest true version of the definition: **a chatbot only talks; an agent actually takes action — it triggers real actions in external systems.** The re-plan loop, while architecturally central, was too much machinery for a 90-second game and got cut. The mechanic that replaced it gives the player something to *do* and makes "acts in the real world" literal (systems light up). This does mean the game no longer dramatizes self-correction/re-planning — a deliberate simplicity trade-off, signed off live.

**Approach:** B (contrast — a chatbot's words trigger nothing; an agent's actions fire real systems) + E (the player builds the agent by choosing and *ordering* its real actions, so sequencing carries a real-world consequence). Format: custom.

**Game — "From chat to agent" (built, approved):** One order runs the whole game — Marco: "one large diavola to 12 Main St, paying by card." **Step 1 (all talk):** the plain chatbot replies with nice words ("you'll love the diavola!") while a row of real systems — Payment / Kitchen / Delivery — stays dark and "asleep." Nothing happened; no pizza is coming. That's the *before*. **Step 2 (build the agent):** the player turns it into an agent by handing it real actions — *check the stock, charge the card, fire it to the kitchen, send out a driver* — tapped into a to-do list **in order**. **Run:** each action fires one at a time and lights up its external system (Payment: charged €14.50; Kitchen: order firing; Delivery: driver en route), with a running "real actions fired" feed. Order matters, because an agent acts in the real world: send the driver before the kitchen's even cooked and it fails funnily ("Luca's parked outside with an empty bag — fix the order"), bouncing the player back to re-sequence. That small dependency puzzle is the only "thinking" beat, and it's enough. The mechanic teaches the whole point with the text removed: dark screens that only wake up when the agent takes a real action. End card keeps the trust point via the definition + why-you-care (an agent that charges cards and dispatches drivers is why guardrails matter — cross-nudge to game 10).

**Status:** built and approved live at `/game/agent` (July 2026). Note this section deliberately supersedes the earlier "The loop" concept described in the correction above. (The old `PROMPT-BUILD-AGENT-GAME.md` build brief described that superseded version and has been deleted.)
...
**Approach:** B (contrast — feel the pain of custom translation first). Format: template.

**Game — "Lost in translation":** One order comes in (1× Margherita, no olives, table 4) and must reach three systems: ordering, POS, delivery. Round 1: each system demands that same order in its own incompatible format — ordering wants a pipe-delimited wire format, POS wants XML tags, delivery wants a proprietary CSV row — and the player hand-translates the identical order into each one, field by field, watching the ugly format build up on screen. Same order, three completely different outputs, by hand, three times. Then a fourth system (loyalty) shows up, demanding yet another format: groan, the lesson arriving. Round 2: one standard payload — the same order written once — and all four systems accept it immediately, unchanged. End: definition + "this is why your AI can plug into calendars, docs, and databases without someone hand-coding a translation for each one."
