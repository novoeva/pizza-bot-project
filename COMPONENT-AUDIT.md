...
**Phase 3 — `TermReveal`. ✅ Built 2026-09-02, awaiting review.** One end screen for all 12 games: success mark → "Snapped onto your bot" → "You just learned the term {name}" (from data) → optional score → definition → "What matters" (3 check lines from `term.keyPoints`) → "Don't" (1 cross line from `term.dont`) → optional aside (the four "Real talk" / "In the real world" callouts) → pinned action. `whyYouCare` is no longer shown: its content was folded into the key points (decision Q7). Depends on `Callout`. Needs `keyPoints` / `dont` written for 12 terms in `terms.json` (copy task, can run in parallel with Phases 1–2). Swap all 12 reveal blocks.
...
| Q7 | `TermReveal` content | **Applied (Phase 3):** `keyPoints[]` (3) + `dont[]` (1) written for all 12 terms; `whyYouCare` folded into the key points and no longer shown. |
