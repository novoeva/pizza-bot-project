...
| 2 | **Message from your bot** | speech bubble, tail top-right, navy robot avatar | white | `bg-surface`; avatar `bg-text` | bot replies. Turns green/red **only** when the reply itself is the verdict (roles 6/7 applied to a bubble). |
| 3 | **Teaching & instructions** (the game tells you something) | reading card (`rounded-lg`), eyebrow + icon; or a tinted box inside `GameIntro` | blue | eyebrow `text-tertiary`; boxes `bg-accent-soft` | `PhaseCard`, "How to play", "What you'll learn", "Your role". Never a bubble. |
...
**Phase 3 — `TermReveal` (1 day + copy).** Depends on `Callout`. Needs `keyPoints` / `dont` written for 12 terms in `terms.json` (copy task, can run in parallel with Phases 1–2). Swap all 12 reveal blocks.
