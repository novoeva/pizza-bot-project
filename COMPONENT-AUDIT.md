...
**Phase 4 — `GameIntro` v2 + misfits onto `GameStage`. ✅ Done and approved 2026-09-02 after 4 review rounds.** intro label is "What it is"; the end screen says the definition again in different words (`term.recap`, label "In other words"); `GameStage` stacks with an even 12px gap on phones. MCP's weak simulation logged as FR-24. **Round 2 ("too many boxes on a phone"):** `PhaseCard` and `ProgressBar` are no longer boxes (plain text with a dashed hairline / a bare bar), and `GameIntro`'s inner insets lost their borders. A game screen on a phone now has one framed card at the top, then only the content. **Round 3:** the progress line stays above both columns (tried under the intro card, rejected), slim and unboxed. Decision (Eva): the intro shows the **full definition up front**, labelled "Definition" (define first, apply during the game, repeat at the end); the `term.about` teaser is no longer shown and FR-2's separate "what you'll learn" line is not needed. Token, Context window, Memory and MCP now run on `GameStage` on every screen (left: intro with definition + role; right: PhaseCard + play; progress strip above), which also closes FR-20. Hallucination gets a PhaseCard like the others. `showHowTo` is off everywhere; each screen's instruction lives in its PhaseCard. Depends on Phase 1 (`PhaseCard` replaces the misfits' `<h1>` instruction cards). Adds `term.learn` copy (FR-2). Token, Context window, Memory, MCP get the left orientation column for every phase; their `<h1>` cards go away.
